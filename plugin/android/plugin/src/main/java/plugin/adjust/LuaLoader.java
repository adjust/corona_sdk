...
public class LuaLoader implements JavaFunction, CoronaRuntimeListener {
    ...
    public int invoke(LuaState L) {
        ...
        NamedJavaFunction[] luaFunctions = new NamedJavaFunction[]{
                ...
                new SetEventTrackingSucceededListenerWrapper(),
                new SetEventTrackingFailedListenerWrapper(),
    ...
    public void onExiting(CoronaRuntime runtime) {
        ...
        CoronaLua.deleteRef(runtime.getLuaState(), sessionTrackingSucceededListener);
        CoronaLua.deleteRef(runtime.getLuaState(), sessionTrackingFailedListener);
        CoronaLua.deleteRef(runtime.getLuaState(), eventTrackingSucceededListener);
    ...
    public int adjust_create(final LuaState L) {
        ...
        adjustConfig.setSdkPrefix("corona4.11.5");
        ...
        if (this.eventTrackingSucceededListener != CoronaLua.REFNIL) {
        ...
        if (this.eventTrackingFailedListener != CoronaLua.REFNIL) {
        ...
        if (this.sessionTrackingSucceededListener != CoronaLua.REFNIL) {
        ...
        if (this.sessionTrackingFailedListener != CoronaLua.REFNIL) {
        ...
        if (CoronaLua.isListener(L, listenerIndex, "ADJUST")) {
            ...
            Adjust.getGoogleAdId(CoronaEnvironment.getCoronaActivity(), new OnDeviceIdsRead() {
                ...
                public void onGoogleAdIdRead(String googleAdId) {
                    dispatchEvent(L, finalListener, EVENT_GET_GOOGLEADID, googleAdId);
                }
            });
        }
    ...
    private int adjust_getAmazonAdId(final LuaState L) {
        ...
        if (CoronaLua.isListener(L, listenerIndex, "ADJUST")) {
            listener = CoronaLua.newRef(L, listenerIndex);
            dispatchEvent(L, listener, EVENT_GET_AMAZONADID, "");
        }
...
//        Adjust.setReferrer(referrer, CoronaEnvironment.getCoronaActivity());
    ...
    private int adjust_setEventTrackingSucceededListener(LuaState L) {
        //Hardcoded listener index for ADJUST
        int listenerIndex = 1;

        if (CoronaLua.isListener(L, listenerIndex, "ADJUST")) {
            this.eventTrackingSucceededListener = CoronaLua.newRef(L, listenerIndex);
        }

        return 0;
    }

    private int adjust_setEventTrackingFailedListener(LuaState L) {
        //Hardcoded listener index for ADJUST
        int listenerIndex = 1;

        if (CoronaLua.isListener(L, listenerIndex, "ADJUST")) {
            this.eventTrackingFailedListener = CoronaLua.newRef(L, listenerIndex);
        }

        return 0;
    }

    private int adjust_setSessionTrackingSucceededListener(LuaState L) {
        //Hardcoded listener index for ADJUST
        int listenerIndex = 1;

        if (CoronaLua.isListener(L, listenerIndex, "ADJUST")) {
            this.sessionTrackingSucceededListener = CoronaLua.newRef(L, listenerIndex);
        }

        return 0;
    }

    private int adjust_setSessionTrackingFailedListener(LuaState L) {
        //Hardcoded listener index for ADJUST
        int listenerIndex = 1;

        if (CoronaLua.isListener(L, listenerIndex, "ADJUST")) {
            this.sessionTrackingFailedListener = CoronaLua.newRef(L, listenerIndex);
        }
    ...
    private class SetEventTrackingSucceededListenerWrapper implements NamedJavaFunction {
        @Override
        public String getName() {
            return "setEventTrackingSucceededListener";
        }

        @Override
        public int invoke(LuaState L) {
            return adjust_setEventTrackingSucceededListener(L);
        }
    }

    private class SetEventTrackingFailedListenerWrapper implements NamedJavaFunction {
        @Override
        public String getName() {
            return "setEventTrackingFailedListener";
        }

        @Override
        public int invoke(LuaState L) {
            return adjust_setEventTrackingFailedListener(L);
        }
    }

    private class SetSessionTrackingSucceededListenerWrapper implements NamedJavaFunction {
        @Override
        public String getName() {
            return "setSessionTrackingSucceededListener";
        }

        @Override
        public int invoke(LuaState L) {
            return adjust_setSessionTrackingSucceededListener(L);
        }
    }

    private class SetSessionTrackingFailedListenerWrapper implements NamedJavaFunction {
        @Override
        public String getName() {
            return "setSessionTrackingFailedListener";
        }

        @Override
        public int invoke(LuaState L) {
            return adjust_setSessionTrackingFailedListener(L);
        }
    }
